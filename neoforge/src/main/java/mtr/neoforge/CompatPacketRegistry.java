...
public class CompatPacketRegistry {
    ...
    public HashMap<Identifier, NetworkUtilities.PacketCallback> packetsC2S = new HashMap<>();
    ...
    public void registerNetworkReceiverC2S(Identifier resourceLocation, NetworkUtilities.PacketCallback consumer) {
        packets.computeIfAbsent(resourceLocation, CompatPacket::new);
    ...
    public void commit(PayloadRegistrar registrar) {
        for (Map.Entry<Identifier, CompatPacket> packets : packets.entrySet()) {
            ...
            registrar.playBidirectional(packet.TYPE, packet.STREAM_CODEC,
                    (arg, iPayloadContext) -> {
                        handlerC2S.packetCallback(
                            ((ServerPlayer) iPayloadContext.player()).level().getServer(),

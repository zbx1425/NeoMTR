...
public class RouteMapGenerator implements IGui {
	...
	public static void scrollTextLightRail(PoseStack matrices, VertexConsumer vertexConsumer, int rows, float availableWidth, float availableHeight, int imageWidth, int imageHeight) {
		...
		IDrawing.drawTexture(matrices.last(), vertexConsumer, Math.max(x, 0), 0, imageWidth * scale + Math.min(x, 0), availableHeight, Math.max(-x, 0) / imageWidth / scale, (float) row / rows, 1, (float) (row + 1) / rows, Direction.UP, ARGB_WHITE, MAX_LIGHT_GLOWING);
	}
	...
	private static void drawResource(NativeImage nativeImage, String resource, int x, int y, int width, int height, boolean flipX, float v1, float v2, int color, boolean useActualColor) throws IOException {
		...
		for (int drawX = 0; drawX < width; drawX++) {
			for (int drawY = Math.round(v1 * height); drawY < Math.round(v2 * height); drawY++) {
				...
				final int pixel1 = nativeImageResource.getPixel(Mth.clamp(floorX, 0, resourceWidth - 1), Mth.clamp(floorY, 0, resourceHeight - 1));
				final int pixel2 = nativeImageResource.getPixel(Mth.clamp(ceilX, 0, resourceWidth - 1), Mth.clamp(floorY, 0, resourceHeight - 1));
				final int pixel3 = nativeImageResource.getPixel(Mth.clamp(floorX, 0, resourceWidth - 1), Mth.clamp(ceilY, 0, resourceHeight - 1));
				final int pixel4 = nativeImageResource.getPixel(Mth.clamp(ceilX, 0, resourceWidth - 1), Mth.clamp(ceilY, 0, resourceHeight - 1));
				final int newColor;
				if (useActualColor) {
	...
	private static void drawPixelSafe(NativeImage nativeImage, int x, int y, int color) {
		if (RailwayData.isBetween(x, 0, nativeImage.getWidth() - 1) && RailwayData.isBetween(y, 0, nativeImage.getHeight() - 1)) {
			nativeImage.setPixel(x, y, invertColor(color));
		}
	}

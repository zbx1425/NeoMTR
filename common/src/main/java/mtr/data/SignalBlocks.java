...
public class SignalBlocks {
	...
	public static class SignalBlock extends NameColorDataBase {
		...
		public SignalBlock(CompoundTag compoundTag) {
			...
			DyeColor savedColor;
			try {
				savedColor = DyeColor.values()[compoundTag.getIntOr(KEY_COLOR, 0)];
			} catch (Exception e) {
				MTR.LOGGER.error("", e);
				savedColor = DyeColor.RED;
			}
			...
			compoundTagRails.keySet().forEach(key -> rails.add(UUID.fromString(compoundTagRails.getStringOr(key, ""))));
		}
